...
<Animation>
	<form class="flex mt-32 justify-center" action="?/test" method="post">
		<div class="card p-4 w-2/3">
			<h3 class="h3 m-4">Select which photos you would like to use, and verify pet information is correct.</h3>
			<div class="flex justify-between flex-row">
				{#each urls as url}
					<label class="flex items-center space-x-2">
						<input class="checkbox" type="checkbox" checked />
						<img class="w-1/3" src={url.data.publicUrl} alt="" />
					</label>
				{/each}
			</div>
